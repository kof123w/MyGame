...
namespace BEPUphysics.DataStructures
{
    ...
    public class MeshBoundingBoxTree
    {
        ...
        /// Gets the triangles whose bounding boxes are overlapped by the query.
        /// </summary>
        /// <param name="fpRay">Shape to query against the tree.</param>
        /// <param name="outputOverlappedElements">Indices of triangles in the index buffer with bounding boxes which are overlapped by the query.</param>
        /// <returns>Whether or not any elements were overlapped.</returns>
        public bool GetOverlaps(FPRay fpRay, IList<int> outputOverlappedElements)
        {
            if (root != null)
            {
                Fix64 result;
                if (fpRay.Intersects(ref root.BoundingBox, out result))
                    root.GetOverlaps(ref fpRay, Fix64.MaxValue, outputOverlappedElements);
            }
            return outputOverlappedElements.Count > 0;
        }
        /// <summary>
        ...
        /// <param name="outputOverlappedElements">Indices of triangles in the index buffer with bounding boxes which are overlapped by the query.</param>
        /// <returns>Whether or not any elements were overlapped.</returns>
        public bool GetOverlaps(FPRay fpRay, Fix64 maximumLength, IList<int> outputOverlappedElements)
        {
            if (root != null)
            {
                Fix64 result;
                if (fpRay.Intersects(ref root.BoundingBox, out result))
                    root.GetOverlaps(ref fpRay, maximumLength, outputOverlappedElements);
            }
            return outputOverlappedElements.Count > 0;
        }
        ...
        abstract class Node
        {
            ...
            internal abstract void GetOverlaps(ref FPRay fpRay, Fix64 maximumLength, IList<int> outputOverlappedElements);
        ...
        sealed class InternalNode : Node
        {
            ...
            internal override void GetOverlaps(ref FPRay fpRay, Fix64 maximumLength, IList<int> outputOverlappedElements)
            {
                Fix64 result;
                if (fpRay.Intersects(ref ChildA.BoundingBox, out result) && result < maximumLength)
                    ChildA.GetOverlaps(ref fpRay, maximumLength, outputOverlappedElements);
                if (fpRay.Intersects(ref ChildB.BoundingBox, out result) && result < maximumLength)
                    ChildB.GetOverlaps(ref fpRay, maximumLength, outputOverlappedElements);
            }
