...
namespace BEPUphysics.BroadPhaseEntries.MobileCollidables
{
    ...
    public class CompoundCollidable : EntityCollidable
    {
        ...
        /// <param name="fpRayHit">Hit location of the ray on the collidable, if any.</param>
        ...
        public override bool RayCast(FPRay fpRay, Fix64 maximumLength, out FPRayHit fpRayHit)
        {
            ...
            bool hit = RayCast(fpRay, maximumLength, out fpRayHit, out hitChild);
            return hit;
        }
        ...
        public bool RayCast(FPRay fpRay, Fix64 maximumLength, out RayCastResult rayHit)
        {
            FPRayHit hitData;
            CompoundChild hitChild;
            bool hit = RayCast(fpRay, maximumLength, out hitData, out hitChild);
        ...
        /// <param name="fpRay">Ray to test.</param>
        /// <param name="maximumLength">Maximum length, in units of the ray's direction's length, to test.</param>
        /// <param name="fpRayHit">Hit data, if any.</param>
        /// <param name="hitChild">Child collidable hit by the ray, if any.</param>
        /// <returns>Whether or not the ray hit the entry.</returns>
        public bool RayCast(FPRay fpRay, Fix64 maximumLength, out FPRayHit fpRayHit, out CompoundChild hitChild)
        {
            fpRayHit = new FPRayHit();
            hitChild = null;
            var hitElements = PhysicsResources.GetCompoundChildList();
            if (hierarchy.Tree.GetOverlaps(fpRay, maximumLength, hitElements))
            {
                fpRayHit.T = Fix64.MaxValue;
                for (int i = 0; i < hitElements.Count; i++)
                {
                    EntityCollidable candidate = hitElements.Elements[i].CollisionInformation;
                    FPRayHit tempHit;
                    if (candidate.RayCast(fpRay, maximumLength, out tempHit) && tempHit.T < fpRayHit.T)
                    {
                        fpRayHit = tempHit;
                ...
                return fpRayHit.T != Fix64.MaxValue;
            }
            PhysicsResources.GiveBack(hitElements);
            return false;
        }
        ...
        /// <param name="fpRayHit">Hit location of the ray on the collidable, if any.</param>
        /// <returns>Whether or not the ray hit the collidable.</returns>
        public override bool RayCast(FPRay fpRay, Fix64 maximumLength, Func<BroadPhaseEntry, bool> filter, out FPRayHit fpRayHit)
        {
            CompoundChild hitChild;
            bool hit = RayCast(fpRay, maximumLength, filter, out fpRayHit, out hitChild);
            return hit;
        }
        ...
        /// in the entry, this filter will be passed into inner ray casts.</param>
        /// <returns>Whether or not the ray hit the entry.</returns>
        public bool RayCast(FPRay fpRay, Fix64 maximumLength, Func<BroadPhaseEntry, bool> filter, out RayCastResult rayHit)
        {
            FPRayHit hitData;
            CompoundChild hitChild;
            bool hit = RayCast(fpRay, maximumLength, filter, out hitData, out hitChild);
            rayHit = new RayCastResult { HitData = hitData, HitObject = hitChild.CollisionInformation };
            return hit;
        }
        ...
        /// <returns>Whether or not the ray hit the collidable.</returns>
        public bool RayCast(FPRay fpRay, Fix64 maximumLength, Func<BroadPhaseEntry, bool> filter, out FPRayHit fpRayHit, out CompoundChild hitChild)
        {
            fpRayHit = new FPRayHit();
            hitChild = null;
            if (filter(this))
            {
                ...
                if (hierarchy.Tree.GetOverlaps(fpRay, maximumLength, hitElements))
                {
                    ...
                    {
                        FPRayHit tempHit;
                        if (hitElements.Elements[i].CollisionInformation.RayCast(fpRay, maximumLength, filter, out tempHit) && tempHit.T < fpRayHit.T)
                        {
                            fpRayHit = tempHit;
                            hitChild = hitElements.Elements[i];
                        }
        ...
        public override bool ConvexCast(CollisionShapes.ConvexShapes.ConvexShape castShape, ref RigidTransform startingTransform, ref FPVector3 sweep, out FPRayHit fpRayHit)
        {
            CompoundChild hitChild;
            bool hit = ConvexCast(castShape, ref startingTransform, ref sweep, out fpRayHit, out hitChild);
            return hit;
        }
        ...
        public bool ConvexCast(CollisionShapes.ConvexShapes.ConvexShape castShape, ref RigidTransform startingTransform, ref FPVector3 sweep, out RayCastResult result)
        {
            ...
            FPRayHit fpRayHit;
            bool hit = ConvexCast(castShape, ref startingTransform, ref sweep, out fpRayHit, out hitChild);
            result = new RayCastResult { HitData = fpRayHit, HitObject = hitChild.CollisionInformation };
            return hit;
        }
        ...
        /// <param name="fpRayHit">Hit data, if any.</param>
        /// <returns>Whether or not the cast hit anything.</returns>
        public override bool ConvexCast(CollisionShapes.ConvexShapes.ConvexShape castShape, ref RigidTransform startingTransform, ref FPVector3 sweep, Func<BroadPhaseEntry, bool> filter, out FPRayHit fpRayHit)
        {
            CompoundChild hitChild;
            bool hit = ConvexCast(castShape, ref startingTransform, ref sweep, filter, out fpRayHit, out hitChild);
            return hit;
        }
