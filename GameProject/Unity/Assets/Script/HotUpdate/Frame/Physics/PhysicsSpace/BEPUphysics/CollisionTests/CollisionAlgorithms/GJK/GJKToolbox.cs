...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms.GJK
{
    ...
    public static class GJKToolbox
    {
        ...
        public static bool RayCast(FPRay fpRay, ConvexShape shape, ref RigidTransform shapeTransform, Fix64 maximumLength,
                                   out FPRayHit hit)
        {
            //Transform the ray into the object's local space.
            FPVector3.Subtract(ref fpRay.origin, ref shapeTransform.Position, out fpRay.origin);
            FPQuaternion conjugate;
            FPQuaternion.Conjugate(ref shapeTransform.Orientation, out conjugate);
            FPQuaternion.Transform(ref fpRay.origin, ref conjugate, out fpRay.origin);
            FPQuaternion.Transform(ref fpRay.direction, ref conjugate, out fpRay.direction);
            ...
            while (closestOffset.LengthSquared() >= Toolbox.Epsilon * simplex.GetErrorTolerance(ref fpRay.origin))
            {
                if (++count > MaximumGJKIterations)
                {
                ...
                if (vw > F64.C0)
                {
                    ...
                    FPVector3.Dot(ref closestOffset, ref fpRay.direction, out closestPointDotDirection);
        ...
        public static bool SphereCast(FPRay fpRay, Fix64 radius, ConvexShape shape, ref RigidTransform shapeTransform, Fix64 maximumLength,
                                   out FPRayHit hit)
        {
            //Transform the ray into the object's local space.
            FPVector3.Subtract(ref fpRay.origin, ref shapeTransform.Position, out fpRay.origin);
            ...
            //This epsilon has a significant impact on performance and accuracy.  Changing it to use BigEpsilon instead increases speed by around 30-40% usually, but jigging is more evident.
            while (v.LengthSquared() >= Toolbox.Epsilon * simplex.GetErrorTolerance(ref fpRay.origin))
            {
                if (++count > MaximumGJKIterations)
                {
                    //It's taken too long to find a hit.  Numerical problems are probable; quit.
                ...
                if (vw > F64.C0)
                {
                    FPVector3.Dot(ref v, ref fpRay.direction, out vdir);
                    hit.T = hit.T - vw / vdir;
                    if (vdir >= F64.C0)
                    {
        ...
        ///<param name="hit">Hit data of the sphere cast, if any.</param>
        ///<returns>Whether or not the sphere cast hit the shape.</returns>
        public static bool CCDSphereCast(FPRay fpRay, Fix64 radius, ConvexShape target, ref RigidTransform shapeTransform, Fix64 maximumLength,
                                   out FPRayHit hit)
        {
            int iterations = 0;
            while (true)
            {
                if (GJKToolbox.SphereCast(fpRay, radius, target, ref shapeTransform, maximumLength, out hit) &&
                    hit.T > F64.C0)
                {
                ...
                if (iterations > 3) //Limit could be configurable.
                {
                    ...
                    return GJKToolbox.RayCast(fpRay, target, ref shapeTransform, maximumLength, out hit) && hit.T > F64.C0;
