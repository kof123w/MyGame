...
namespace BEPUphysics.CollisionShapes
{
    ...
    public class MobileMeshShape : EntityShape
    {
        ...
        public MobileMeshShape(TransformableMeshData meshData, IList<FPVector3> hullVertices, MobileMeshSolidity solidity, TriangleSidedness sidednessWhenSolid, Fix64 collisionMargin, EntityShapeVolumeDescription volumeDescription)
        {
        ...
        /// </summary>
        /// <param name="fpRay">Ray in the local space of the shape to test.</param>
        /// <param name="hit">The first hit against the mesh, if any.</param>
        /// <returns>Whether or not the ray origin was in the mesh.</returns>
        public bool IsLocalRayOriginInMesh(ref FPRay fpRay, out FPRayHit hit)
        {
            var overlapList = CommonResources.GetIntList();
            hit = new FPRayHit();
            hit.T = Fix64.MaxValue;
            if (triangleMesh.Tree.GetOverlaps(fpRay, overlapList))
            {
                ...
                for (int i = 0; i < overlapList.Count; i++)
                {
                    ...
                    bool hitClockwise;
                    FPRayHit tempHit;
                    if (Toolbox.FindRayTriangleIntersection(ref fpRay, Fix64.MaxValue, ref vA, ref vB, ref vC, out hitClockwise, out tempHit) &&
                        tempHit.T < hit.T)
                    {
                        hit = tempHit;
        ...
        TriangleSidedness ComputeSolidSidednessHelper(FPRay fpRay)
        {
            TriangleSidedness toReturn;
            var hitList = CommonResources.GetIntList();
            if (triangleMesh.Tree.GetOverlaps(fpRay, hitList))
            {
                FPVector3 vA, vB, vC;
                ...
                for (int i = 0; i < hitList.Count; i++)
                {
                    ...
                    FPRayHit hit;
                    if (Toolbox.FindRayTriangleIntersection(ref fpRay, Fix64.MaxValue, TriangleSidedness.DoubleSided, ref vA, ref vB, ref vC, out hit) &&
                ...
                if (hits.Count % 2 == 0)
                {
                    ...
                    if (FPVector3.Dot(normal, fpRay.direction) < F64.C0)
        ...
        private ShapeDistributionInformation ComputeVolumeDistribution(TransformableMeshData data)
        {
            ...
            shapeInformation.VolumeDistribution = new FPMatrix3x3();
            ...
            for (int i = 0; i < data.indices.Length; i += 3)
            {
                ...
                FPMatrix3x3 aContribution, bContribution, cContribution;
                ...
                FPMatrix3x3.Add(ref aContribution, ref shapeInformation.VolumeDistribution, out shapeInformation.VolumeDistribution);
                FPMatrix3x3.Add(ref bContribution, ref shapeInformation.VolumeDistribution, out shapeInformation.VolumeDistribution);
                FPMatrix3x3.Add(ref cContribution, ref shapeInformation.VolumeDistribution, out shapeInformation.VolumeDistribution);
            ...
            FPMatrix3x3.Multiply(ref shapeInformation.VolumeDistribution, F64.C1 / (F64.C2 * totalWeight), out shapeInformation.VolumeDistribution);

            //Move the inertia tensor into position according to the center.
            FPMatrix3x3 additionalInertia;
            InertiaHelper.GetPointContribution(F64.C0p5, ref Toolbox.ZeroVector, ref shapeInformation.Center, out additionalInertia);
            FPMatrix3x3.Subtract(ref shapeInformation.VolumeDistribution, ref additionalInertia, out shapeInformation.VolumeDistribution);
        ...
        private void GetBoundingBox(ref FPMatrix3x3 o, out BoundingBox boundingBox)
        {
#if !WINDOWS
            ...
            FPVector3.Multiply(ref rightDirection, meshCollisionMargin / Fix64.Sqrt(rightDirection.Length()), out rightDirection);
            FPVector3.Multiply(ref upDirection, meshCollisionMargin / Fix64.Sqrt(upDirection.Length()), out upDirection);
            FPVector3.Multiply(ref backDirection, meshCollisionMargin / Fix64.Sqrt(backDirection.Length()), out backDirection);
        ...
        /// <param name="boundingBox">Bounding box in the local space.</param>
        public void GetSweptLocalBoundingBox(ref RigidTransform shapeTransform, ref AffineTransform spaceTransform, ref FPVector3 sweep, out BoundingBox boundingBox)
        {
            GetLocalBoundingBox(ref shapeTransform, ref spaceTransform, out boundingBox);
            FPVector3 expansion;
            FPMatrix3x3.TransformTranspose(ref sweep, ref spaceTransform.LinearTransform, out expansion);
