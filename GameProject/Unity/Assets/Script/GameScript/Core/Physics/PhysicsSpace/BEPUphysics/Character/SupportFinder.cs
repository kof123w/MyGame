    ...
    public class SupportFinder
    {
        ...
        private void UpdateSupportData(ref FPVector3 down)
        {
            ...
            if (contacts.Count > 1)
            {
                ...
                {
                    FPVector3.Multiply(ref supportData.Normal, F64.C1 / Fix64.Sqrt(length), out supportData.Normal);
                }
            }
        ...
        public void UpdateSupports(ref FPVector3 movementDirection)
        {
            ...
            if (!HasSupport && hadTraction)
            {
                ...
                if (!HasTraction)
                {
                    ...
                    FPRay fpRay = new FPRay(bodyPosition, downDirection);

                    bool hasTraction;
                    SupportRayData data;
                    if (TryDownCast(ref fpRay, supportRayLength, out hasTraction, out data))
                    {
                        SupportRayData = data;
                ...
                if (!HasTraction && tryingToMove)
                {
                    ...
                    FPRay obstructionFpRay;
                    obstructionFpRay.origin = characterBody.Position;
                    obstructionFpRay.direction = fpRay.origin - obstructionFpRay.origin;
                    if (!QueryManager.RayCastHitAnything(obstructionFpRay, F64.C1))
                    {
                ...
                if (!HasTraction && tryingToMove)
                {
                    ...
                    FPRay fpRay = new FPRay(bodyPosition + horizontalOffset, downDirection);

                    //Have to test to make sure the ray doesn't get obstructed.  This could happen if the character is deeply embedded in a wall; we wouldn't want it detecting things inside the wall as a support!
                    FPRay obstructionFpRay;
                    obstructionFpRay.origin = bodyPosition;
                    obstructionFpRay.direction = fpRay.origin - obstructionFpRay.origin;
                    if (!QueryManager.RayCastHitAnything(obstructionFpRay, F64.C1))
                    {
                        //The origin isn't obstructed, so now ray cast down.
        ...
        bool TryDownCast(ref FPRay fpRay, Fix64 length, out bool hasTraction, out SupportRayData supportRayData)
        {
            FPRayHit earliestHit;
            Collidable earliestHitObject;
            supportRayData = new SupportRayData();
            hasTraction = false;
            if (QueryManager.RayCast(fpRay, length, out earliestHit, out earliestHitObject))
            {
                ...
                FPVector3.Divide(ref earliestHit.Normal, Fix64.Sqrt(lengthSquared), out earliestHit.Normal);
                //A collidable was hit!  It's a support, but does it provide traction?
                earliestHit.Normal.Normalize();
                Fix64 dot;
                FPVector3.Dot(ref fpRay.direction, ref earliestHit.Normal, out dot);
                if (dot < F64.C0)
                {
        ...
        internal bool IsSideContactObstructive(ref ContactData contact)
        {
            ...
            foreach (var c in SideContacts)
            {
                ...
                Fix64 dot = FPVector3.Dot(contact.Normal, c.Contact.Normal);
