...
namespace BEPUphysics.Constraints.SolverGroups
{
    ...
    public abstract class SolverGroup : SolverUpdateable
    {
        ...
        /// </summary>
        /// <param name="newBepUphysicsSpace"></param>
        public override void OnAdditionToSpace(BEPUphysicsSpace newBepUphysicsSpace)
        {
            for (int i = 0; i < solverUpdateables.Count; i++)
            {
                solverUpdateables[i].OnAdditionToSpace(newBepUphysicsSpace);
            }
        }
        ...
        public override void OnRemovalFromSpace(BEPUphysicsSpace oldBepUphysicsSpace)
        {
            for (int i = 0; i < solverUpdateables.Count; i++)
            {
                solverUpdateables[i].OnRemovalFromSpace(oldBepUphysicsSpace);
            }
        }
