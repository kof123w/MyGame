...
namespace BEPUphysics
{
    ...
    public class BEPUphysicsSpace
    {
        ...
        public BEPUphysicsSpace(IParallelLooper parallelLooper)
        {
        ...
        {
            if (spaceObject.BepUphysicsSpace != this)
            ...
            spaceObject.BepUphysicsSpace = null;
        ...
        public bool RayCast(FPRay fpRay, out RayCastResult result)
        {
            return RayCast(fpRay, Fix64.MaxValue, out result);
        }
        ...
        /// <param name="result">Hit data of the ray, if any.</param>
        /// <returns>Whether or not the ray hit anything.</returns>
        public bool RayCast(FPRay fpRay, Func<BroadPhaseEntry, bool> filter, out RayCastResult result)
        {
            return RayCast(fpRay, Fix64.MaxValue, filter, out result);
        }
        ...
        /// <param name="result">Hit data of the ray, if any.</param>
        /// <returns>Whether or not the ray hit anything.</returns>
        public bool RayCast(FPRay fpRay, Fix64 maximumLength, out RayCastResult result)
        {
            var resultsList = PhysicsResources.GetRayCastResultList();
            bool didHit = RayCast(fpRay, maximumLength, resultsList);
        ...
        /// <param name="result">Hit data of the ray, if any.</param>
        /// <returns>Whether or not the ray hit anything.</returns>
        public bool RayCast(FPRay fpRay, Fix64 maximumLength, Func<BroadPhaseEntry, bool> filter, out RayCastResult result)
        {
            var resultsList = PhysicsResources.GetRayCastResultList();
            bool didHit = RayCast(fpRay, maximumLength, filter, resultsList);
            result = resultsList.Elements[0];
            for (int i = 1; i < resultsList.Count; i++)
            {
                RayCastResult candidate = resultsList.Elements[i];
        ...
        /// <returns>Whether or not the ray hit anything.</returns>
        public bool RayCast(FPRay fpRay, Fix64 maximumLength, IList<RayCastResult> outputRayCastResults)
        {
            var outputIntersections = PhysicsResources.GetBroadPhaseEntryList();
            if (BroadPhase.QueryAccelerator.RayCast(fpRay, maximumLength, outputIntersections))
            {
                ...
                for (int i = 0; i < outputIntersections.Count; i++)
                {
                    ...
                    if (candidate.RayCast(fpRay, maximumLength, out fpRayHit))
                    {
                        outputRayCastResults.Add(new RayCastResult(fpRayHit, candidate));
                    }
                }
            }
        ...
        /// <param name="outputRayCastResults">Hit data of the ray, if any.</param>
        /// <returns>Whether or not the ray hit anything.</returns>
        public bool RayCast(FPRay fpRay, Fix64 maximumLength, Func<BroadPhaseEntry, bool> filter, IList<RayCastResult> outputRayCastResults)
        {
            var outputIntersections = PhysicsResources.GetBroadPhaseEntryList();
            if (BroadPhase.QueryAccelerator.RayCast(fpRay, maximumLength, outputIntersections))
            {
                ...
                for (int i = 0; i < outputIntersections.Count; i++)
                {
                    FPRayHit fpRayHit;
                    BroadPhaseEntry candidate = outputIntersections.Elements[i];
                    if (candidate.RayCast(fpRay, maximumLength, filter, out fpRayHit))
                    {
                        outputRayCastResults.Add(new RayCastResult(fpRayHit, candidate));
                    }
                }
