...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public static class MPRToolbox
    {
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref FPVector3 direction, out Fix64 t, out FPVector3 normal)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon * F64.C0p01)
                    FPVector3.Divide(ref direction, Fix64.Sqrt(rayLengthSquared), out normal);
                else
            ...
            while (true)
            {
                ...
                if (supportDot - dot < surfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    {
                        FPVector3.Divide(ref n, Fix64.Sqrt(lengthSquared), out normal);
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref FPVector3 direction, out Fix64 t, out FPVector3 normal, out FPVector3 position)
        {
            // Local surface cast is very similar to regular MPR.  However, instead of starting at an interior point and targeting the origin,
            // the ray starts at the origin (a point known to be in both shape and shapeB), and just goes towards the direction until the surface
            // is found.  The portal (v1, v2, v3) at termination defines the surface normal, and the distance from the origin to the portal along the direction is used as the 't' result.
        ...
        static bool VerifySimplex(ref FPVector3 v0, ref FPVector3 v1, ref FPVector3 v2, ref FPVector3 v3, ref FPVector3 direction)
        {
            ...
            Fix64 planeProduct1 = FPVector3.Dot(cross, direction);
            //v3, v0, v2
            cross = FPVector3.Cross(v0 - v3, v2 - v3);
            Fix64 planeProduct2 = FPVector3.Dot(cross, direction);
            //v2, v0, v1
            cross = FPVector3.Cross(v0 - v2, v1 - v2);
            Fix64 planeProduct3 = FPVector3.Dot(cross, direction);
        ...
        public static bool GetContact(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB, ref FPVector3 penetrationAxis, out ContactData contact)
        {
            ...
            if (MPRToolbox.AreLocalShapesOverlapping(shapeA, shapeB, ref localTransformB))
            {
                ...
                {
                    FPVector3.Divide(ref penetrationAxis, Fix64.Sqrt(lengthSquared), out rayCastDirection);// (Vector3.Normalize(localDirection) + Vector3.Normalize(collidableB.worldTransform.Position - collidableA.worldTransform.Position)) / 2;
                ...
                if (lengthSquared > Toolbox.Epsilon)
                {
                    FPVector3.Divide(ref localTransformB.Position, Fix64.Sqrt(lengthSquared), out rayCastDirection);
                ...
                FPMatrix3x3.CreateFromQuaternion(ref transformA.Orientation, out orientation);
                FPMatrix3x3.Transform(ref contact.Normal, ref orientation, out contact.Normal);
                //Vector3.Negate(ref contact.Normal, out contact.Normal);
                FPMatrix3x3.Transform(ref contact.Position, ref orientation, out contact.Position);
        ...
        public static void RefinePenetration(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, Fix64 initialDepth, ref FPVector3 initialNormal, out Fix64 penetrationDepth, out FPVector3 refinedNormal, out FPVector3 position)
        {
            //The local casting can optionally continue.  Eventually, it will converge to the local minimum.
        ...
        private static bool LocalSweepCast(ConvexShape shape, ConvexShape shapeB, Fix64 sweepLength, Fix64 rayLengthSquared, ref FPVector3 localDirection, ref FPVector3 sweep, ref RigidTransform localTransformB, out FPRayHit hit)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon * F64.C0p01)
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon * F64.C0p01)
                    FPVector3.Divide(ref localDirection, Fix64.Sqrt(rayLengthSquared), out hit.Normal);
                else
                    hit.Normal = new FPVector3();
            ...
            while (true)
            {
                ...
                if (supportDot - dot < rayCastSurfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    {
                        FPVector3.Divide(ref n, Fix64.Sqrt(lengthSquared), out hit.Normal);
        ...
        internal static void GetLocalPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref FPVector3 minkowskiPosition, out FPVector3 position)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                Fix64 dot = FPVector3.Dot(v1 - minkowskiPosition, rayDirection);
                ...
                //Origin is inside.
                //Compute barycentric coordinates along simplex (segment).
                Fix64 dotv0 = FPVector3.Dot(v0 - minkowskiPosition, rayDirection);
                //Dot > 0, so dotv0 starts out negative.
        ...
        /// <returns>Whether or not the swept shapes hit each other.</returns>
        public static bool RayCast(FPRay fpRay, Fix64 maximumLength, ConvexShape shape, ref RigidTransform transform, out FPRayHit hit)
        {
            ...
            FPVector3.Negate(ref localFpRay.direction, out localFpRay.direction);
            ...
            if (Toolbox.RayCastSphere(ref fpRay, ref transform.Position, shape.MaximumRadius, maximumLength, out sphereHit))
            {
                ...
                FPVector3 offset;
                FPVector3.Multiply(ref localFpRay.direction, -sphereHit.T, out offset);
                FPVector3.Add(ref localFpRay.origin, ref offset, out localFpRay.origin);
            }
            else
            {
            ...
            Fix64 rayLengthSquared = localFpRay.direction.LengthSquared();
            Fix64 sweepLength;
            if (rayLengthSquared > Toolbox.Epsilon * F64.C0p01)
            {
                FPVector3.Dot(ref localFpRay.origin, ref localFpRay.direction, out sweepLength);
            ...
            FPVector3 sweep;
            FPVector3.Multiply(ref localFpRay.direction, sweepLength, out sweep);
            //Check to see if the origin is contained within the swept shape.
            if (!SweptShapeContainsPoint(shape, ref sweep, ref localFpRay.origin))
            {
                //The origin is not contained within the sweep volume.  The raycast definitely misses.
                hit.T = Fix64.MaxValue;
                hit.Normal = new FPVector3();
            ...
            if (negativeLength)
            {
                ...
                hit.T = F64.C0;
                FPVector3.Normalize(ref fpRay.direction, out hit.Normal);
                hit.Location = fpRay.origin;
                return true;
            }

            //OKAY! We've finally finished all the pre-testing.  Cast the ray!
            if (LocalSweepCast(shape, sweepLength, rayLengthSquared, ref localFpRay.direction, ref sweep, ref localFpRay.origin, out hit))
            {
                hit.T += sphereHit.T;
        ...
        private static bool LocalSweepCast(ConvexShape shape, Fix64 sweepLength, Fix64 rayLengthSquared, ref FPVector3 localDirection, ref FPVector3 sweep, ref FPVector3 rayOrigin, out FPRayHit hit)
        {
            //By now, the ray is known to be within the swept shape and facing the right direction for a normal raycast.
