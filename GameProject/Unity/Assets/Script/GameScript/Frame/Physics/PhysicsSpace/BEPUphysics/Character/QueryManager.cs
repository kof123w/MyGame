...
namespace BEPUphysics.Character
{
    ...
    public class QueryManager
    {
        ...
        public bool RayCast(FPRay fpRay, Fix64 length, out FPRayHit earliestHit)
        {
            ...
            foreach (var collidable in characterBody.CollisionInformation.OverlappedCollidables)
            {
                ...
                Fix64 t;
                if (fpRay.Intersects(ref collidable.boundingBox, out t) && t < length)
                {
                    //Is it an earlier hit than the current earliest?
                    FPRayHit hit;
                    if (collidable.RayCast(fpRay, length, SupportRayFilter, out hit) && hit.T < earliestHit.T)
                    {
                        earliestHit = hit;
                    }
                }
        ...
        /// <returns>Whether or not the ray hit anything.</returns>
        public bool RayCast(FPRay fpRay, Fix64 length, out FPRayHit earliestHit, out Collidable hitObject)
        {
            earliestHit = new FPRayHit();
            earliestHit.T = Fix64.MaxValue;
            hitObject = null;
        ...
        /// <param name="length">Length of the ray to use in units of the ray's length.</param>
        /// <returns>Whether or not the ray hit anything.</returns>
        public bool RayCastHitAnything(FPRay fpRay, Fix64 length)
        {
            foreach (var collidable in characterBody.CollisionInformation.OverlappedCollidables)
            {
                //Check to see if the collidable is hit by the ray.
                ...
                {
                    FPRayHit hit;
                    if (collidable.RayCast(fpRay, length, SupportRayFilter, out hit))
                    {
                        return true;
                    }
                }
