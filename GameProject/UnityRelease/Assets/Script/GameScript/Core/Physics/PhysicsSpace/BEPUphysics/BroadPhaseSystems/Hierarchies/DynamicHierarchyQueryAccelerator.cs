...
namespace BEPUphysics.BroadPhaseSystems.Hierarchies
{
    ...
    public class DynamicHierarchyQueryAccelerator : IQueryAccelerator
    {
        ...
        /// </summary>
        /// <param name="fpRay">Ray to test against the structure.</param>
        ...
        public bool RayCast(FPRay fpRay, Fix64 maximumLength, IList<BroadPhaseEntry> entries)
        {
            if (hierarchy.root != null)
            {
                hierarchy.root.GetOverlaps(ref fpRay, maximumLength, entries);
        ...
        /// <param name="fpRay">Ray to test against the structure.</param>
        /// <param name="entries">Entries which have bounding boxes that overlap the ray.</param>
        public bool RayCast(FPRay fpRay, IList<BroadPhaseEntry> entries)
        {
            if (hierarchy.root != null)
            {
                hierarchy.root.GetOverlaps(ref fpRay, Fix64.MaxValue, entries);
