...
namespace BEPUphysics.DataStructures
{
    ...
    public class BoundingBoxTree<T> where T : IBoundingBoxOwner
    {
        ...
        /// <param name="outputOverlappedElements">Indices of triangles in the index buffer with bounding boxes which are overlapped by the query.</param>
        /// <returns>Whether or not any elements were overlapped.</returns>
        public bool GetOverlaps(FPRay fpRay, IList<T> outputOverlappedElements)
        {
            if (root != null)
            {
                Fix64 result;
        ...
        /// <param name="outputOverlappedElements">Indices of triangles in the index buffer with bounding boxes which are overlapped by the query.</param>
        /// <returns>Whether or not any elements were overlapped.</returns>
        public bool GetOverlaps(FPRay fpRay, Fix64 maximumLength, IList<T> outputOverlappedElements)
        {
            if (root != null)
            {
                Fix64 result;
        ...
        internal abstract class Node
        {
            ...
            internal abstract void GetOverlaps(ref FPRay fpRay, Fix64 maximumLength, IList<T> outputOverlappedElements);
        ...
        internal sealed class InternalNode : Node
        {
            ...
            internal override void GetOverlaps(ref FPRay fpRay, Fix64 maximumLength, IList<T> outputOverlappedElements)
            {
                Fix64 result;
