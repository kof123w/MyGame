...
namespace BEPUphysics.Character
{
    ...
    public class StepManager
    {
        ...
        public bool TryToStepDown(out FPVector3 newPosition)
        {
            ...
            FPRay fpRay;
            characterBody.CollisionInformation.Shape.GetExtremePoint(normal, ref transform, out fpRay.origin);
            fpRay.direction = down;

            //Intersect the ray against the plane defined by the support hit.
            FPVector3 intersection;
            FPPlane fpPlane = new FPPlane(normal, FPVector3.Dot(SupportFinder.SupportRayData.Value.HitData.Location, normal));
            ...
            try
            {
                ...
                Fix64 hitT;
                if (Toolbox.GetRayPlaneIntersection(ref fpRay, ref fpPlane, out hitT, out intersection))
                {
        ...
        bool TryToStepUsingContact(ref ContactData contact, ref FPVector3 down, out FPVector3 newPosition)
        {
            ...
            FPVector3.Multiply(ref down, characterBody.Height * F64.C0p5 - downRayLength, out fpRay.origin);
            FPVector3.Add(ref fpRay.origin, ref position, out fpRay.origin);
            fpRay.direction = normal;
            ...
            if (QueryManager.RayCastHitAnything(fpRay, length))
            {
            ...
            FPVector3.Add(ref fpRay.origin, ref horizontalOffset, out fpRay.origin);
            fpRay.direction = down;

            //Find the earliest hit, if any.
            FPRayHit earliestHit;
            if (!QueryManager.RayCast(fpRay, downRayLength, out earliestHit) || //Can't do anything if it didn't hit.
            ...
            FPVector3.Negate(ref down, out fpRay.direction);
            //Find the earliest hit, if any.
            ...
            if (QueryManager.RayCastHitAnything(fpRay, upLength))
            {
                newPosition = new FPVector3();
                return false;
            }
            ...
            //Use it as the ray origin.
            FPRay downFpRay;
            characterBody.CollisionInformation.Shape.GetExtremePoint(supportNormal, ref transform, out downFpRay.origin);
            downFpRay.direction = down;

            //Intersect the ray against the plane defined by the support hit.
            FPVector3 intersection;
            FPVector3.Dot(ref earliestHit.Location, ref supportNormal, out dot);
            FPPlane fpPlane = new FPPlane(supportNormal, dot);
            FPVector3 candidatePosition;
            ...
            try
            {
                //This guess may either win immediately, or at least give us a better idea of where to search.
                Fix64 hitT;
                if (Toolbox.GetRayPlaneIntersection(ref downFpRay, ref fpPlane, out hitT, out intersection))
                {
            ...
            if (!obstructed && supportContacts.Count > 0)
            {
                ...
                if (supportState == CharacterContactPositionState.Accepted)
                {
                    ...
                    else
                    {
                        ...
                        FPRay downFpRay;
                        downFpRay.origin = supportContact.Contact.Position + sideNormal * F64.C0p001;
                        Fix64 verticalOffset = FPVector3.Dot(downFpRay.origin - position, down);
                        verticalOffset = characterBody.Height * F64.C0p5 + verticalOffset;
                        downFpRay.origin -= verticalOffset * down;
                        downFpRay.direction = down;
                        ...
                        FPRay obstructionTestFpRay;
                        obstructionTestFpRay.origin = position - down * (characterBody.Height * F64.C0p5);
                        obstructionTestFpRay.direction = downFpRay.origin - obstructionTestFpRay.origin;

                        if (!QueryManager.RayCastHitAnything(obstructionTestFpRay, F64.C1))
                        {
                            //Okay! it's safe to cast down, then.
                            FPRayHit hit;
                            if (QueryManager.RayCast(downFpRay, characterBody.Height, out hit))
                            {
                                //Got a hit!
                                if (characterBody.Height - maximumStepHeight < hit.T)
                                {
                                    ...
                                    if (Fix64.Abs(dot) > ContactCategorizer.TractionThreshold)
                                    {
                                        ...
                                        downFpRay.origin = position;
                                        if (QueryManager.RayCast(downFpRay, characterBody.Height * F64.C0p5 + maximumStepHeight, out hit))
                                        {
                                            //It hit.. almost there!
                                            hit.Normal.Normalize();
                                            FPVector3.Dot(ref hit.Normal, ref down, out dot);
