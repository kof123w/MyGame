...
namespace BEPUphysics.CollisionShapes
{
    ...
    public class StaticGroupShape : CollisionShape
    {
        ...
        public bool RayCast(FPRay fpRay, Fix64 maximumLength, out RayCastResult result)
        {
            ...
            CollidableTree.GetOverlaps(fpRay, maximumLength, outputOverlappedElements);
            result = new RayCastResult();
            result.HitData.T = Fix64.MaxValue;
            for (int i = 0; i < outputOverlappedElements.Count; ++i)
            {
                FPRayHit hit;
                if (outputOverlappedElements.Elements[i].RayCast(fpRay, maximumLength, out hit))
                {
        ...
        public bool RayCast(FPRay fpRay, Fix64 maximumLength, Func<BroadPhaseEntry, bool> filter, out RayCastResult result)
        {
            ...
            {
                FPRayHit hit;
                if (outputOverlappedElements.Elements[i].RayCast(fpRay, maximumLength, filter, out hit))
                {
                    if (hit.T < result.HitData.T)
                    {
                        result.HitData = hit;
