    ...
    public class Wheel
    {
        ...
        public FPVector3 LocalForwardDirection
        {
            ...
            set
            {
                ...
                if (vehicle != null)
                    FPMatrix3x3.Transform(ref localForwardDirection, ref Vehicle.Body.orientationMatrix, out worldForwardDirection);
                else
        ...
        public WheelShape Shape
        {
            ...
            set
            {
                if (shape != null)
                {
                    ...
                    if (vehicle != null && vehicle.BepUphysicsSpace != null)
                        shape.OnRemovalFromSpace(vehicle.BepUphysicsSpace);
                    shape.Wheel = null;
                }
                if (value != null)
                {
                    if (value.Wheel == null)
                    {
                        ...
                        if (vehicle != null && vehicle.BepUphysicsSpace != null)
                            value.OnAdditionToSpace(vehicle.BepUphysicsSpace);
                    }
                    else
        ...
        {
            FPMatrix.CreateFromAxisAngle(ref suspension.localDirection, shape.steeringAngle, out shape.steeringTransform);
            FPMatrix.TransformNormal(ref localForwardDirection, ref shape.steeringTransform, out worldForwardDirection);
            FPMatrix3x3.Transform(ref worldForwardDirection, ref Vehicle.Body.orientationMatrix, out worldForwardDirection);
            if (HasSupport)
            {
        ...
        internal void OnAdditionToSpace(BEPUphysicsSpace bepUphysicsSpace)
        {
            //Make sure it doesn't collide with anything.

            shape.OnAdditionToSpace(bepUphysicsSpace);

        }



        internal void OnRemovalFromSpace(BEPUphysicsSpace bepUphysicsSpace)
        {
            shape.OnRemovalFromSpace(bepUphysicsSpace);
        }

        internal void OnAddedToVehicle(Vehicle vehicle)
        {
            this.vehicle = vehicle;
            BEPUphysicsSpace bepUphysicsSpace = (vehicle as ISpaceUpdateable).BepUphysicsSpace;
            if (bepUphysicsSpace != null)
            {
                OnAdditionToSpace(bepUphysicsSpace);
            }
        ...
        internal void OnRemovedFromVehicle()
        {
            ...
            if (bepUphysicsSpace != null)
            {
                OnRemovalFromSpace(bepUphysicsSpace);
            }
            vehicle = null;
        }
